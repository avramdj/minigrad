...
def test_add():
    ...
    assert np.allclose(a.grad.detach().numpy(), a1.grad.detach().numpy())
    assert np.allclose(b.grad.detach().numpy(), b1.grad.detach().numpy())
...
def test_add():
    ...
    assert np.allclose(a.grad.detach().numpy(), a1.grad.detach().cpu())
    assert np.allclose(b.grad.detach().numpy(), b1.grad.detach().cpu())
