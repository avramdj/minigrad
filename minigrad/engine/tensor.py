...
class Tensor:
    ...
    @staticmethod
    def ones(shape: Union[int, Iterable[int]], device=None, requires_grad=True):
        device = device if device else Device.CURRENT
        return Tensor(np.ones(shape), device=device, requires_grad=requires_grad)

    @staticmethod
    def zeros(shape: Union[int, Iterable[int]], device=None, requires_grad=True):
        device = device if device else Device.CURRENT
        return Tensor(np.zeros(shape), device=device, requires_grad=requires_grad)

    @staticmethod
    def rand(shape: Union[int, Iterable[int]], device=None, requires_grad=True):
        device = device if device else Device.CURRENT
        return Tensor(np.random.rand(*shape), device=device, requires_grad=requires_grad)

    @staticmethod
    def full(shape: Union[int, Iterable[int]], a: Union[int, float], device=None, requires_grad=True):
        device = device if device else Device.CURRENT
        return Tensor(np.full(shape, a), device=device, requires_grad=requires_grad)

    @staticmethod
    def ones_like(a: Union[np.ndarray, "Tensor"], device=None, requires_grad=True):
        device = device if device else a.device
        if isinstance(a, Tensor):
            a = a.data
        return Tensor(np.ones_like(a, dtype=np.float32), device, requires_grad=requires_grad)

    @staticmethod
    def rand_like(a: Union[np.ndarray, "Tensor"], device=None, requires_grad=True):
        device = device if device else a.device
        if isinstance(a, Tensor):
            a = a.data
        return Tensor(np.random.rand(a, dtype=np.float32), device, requires_grad=requires_grad)

    @staticmethod
    def zeros_like(a: Union[np.ndarray, "Tensor"], device=None, requires_grad=True):
        device = device if device else a.device
        if isinstance(a, Tensor):
            a = a.data
        return Tensor(np.zeros_like(a, dtype=np.float32), device, requires_grad=requires_grad)
